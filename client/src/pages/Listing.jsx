      ...
      <div>
        ...
        <div className="flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4">
         <p className="text-2xl font-semibold">
          {listing.name} - ${' '} {listing.offer ? listing.discountPrice.toLocaleString('en-US'):listing.regulerPrice.toLocaleString('en-US')}
         ...
         <div className="flex gap-4">
          ...
          {listing.offer && (
            <p className="bg-green-900 w-full max-w-[200px] text-white text-center p-1 rounded-md">${+listing.regulerPrice-+listing.discountPrice} OFF</p>
          )}
         ...
         <ul className="text-green-900 font-semibold text-sm flex flex-wrap gap-4 items-center sm:gap-6">
          ...
          <li className="flex items-center gap-1 whitespace-nowrap"> <FaBath className="text-lg"/>{listing.bedrooms>1 ? `${listing.bathrooms} bath rooms`:`${listing.bathrooms} bath room`} </li>
         ...
         (<button
          disabled={loading}
          className=" bg-slate-700 text-white p-3 rounded-lg uppercase hover:opacity-85 disabled:opacity-80 hover:scale-105 transition duration-200 animate-pulse w-fit" onClick={()=>{setContact(true)}}
        >Contact LandLoard</button>)
         }
